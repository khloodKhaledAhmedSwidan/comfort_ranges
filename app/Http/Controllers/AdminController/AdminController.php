...
//            'password_confirm' => 'required_with:password|same:password|min:4',
    ...
    public function create()
    {
        ...
        if ($roles) {
            return view('admin.admins.admins.create',compact('roles'));
        } else {
    ...
    public function edit($id)
    {
            ...
            $data = Admin::find($id);
        $role =\App\Models\Role::pluck('display_name_ar', 'id')->all();

        return view('admin.admins.admins.edit', compact('data','role'));
...
//        $request['password'] = Hash::make($request->password);
     ...
     $admin =   Admin::where('id',$id)->first()->update($request->all());
     $admin = Admin::find($id);
