                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="portlet light ">
                            ...
                            <form role="form" action="{{route('update_waitedOrderPage',$order->id)}}" method="post"
                                     ...
                                     @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="date" placeholder="@lang('messages.date')  " class="form-control"
                                                   value="{{$order->timeOrders()->orderBy('id','desc')->first()->date}}"/>
                                            @if ($errors->has('date'))
                                                <span class="help-block">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="order_shift_id">
                                                ...
                                                <option
                                                    value="{{\App\Models\OrderShift::where('id' ,$order->timeOrders()->orderBy('id','desc')->first()->order_shift_id )->first()->id}}"
                                                    selected>@lang('messages.from'){{\App\Models\OrderShift::where('id' ,$order->timeOrders()->orderBy('id','desc')->first()->order_shift_id )->first()->from }}@lang('messages.to'){{\App\Models\OrderShift::where('id' ,$order->timeOrders()->orderBy('id','desc')->first()->order_shift_id )->first()->to}}</option>
                                                @foreach(\App\Models\OrderShift::all() as $shift)
                                                    @if ($shift->id != $order->timeOrders()->orderBy('id','desc')->first()->order_shift_id   )
                                                        <option
